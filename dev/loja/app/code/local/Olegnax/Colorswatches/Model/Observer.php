 ...
 */
class Olegnax_Colorswatches_Model_Observer extends Mage_Core_Model_Abstract
{
    /**
    ...
    {
        if (!Mage::helper('olegnaxcolorswatches')->isEnabled()) { // check if functionality disabled
        ...
        $helper->attachProductChildrenAttributeMapping($products, $collection->getStoreId());
        ...
        foreach ($products as $product) {
            ...
            Mage::helper('olegnaxcolorswatches/productimg')
    ...
    public function convertLayerBlock(Varien_Event_Observer $observer)
    {
        ...
        if (($front == 'catalog' && $controller == 'category' && $action == 'view')
            ...
            Mage::helper('olegnaxcolorswatches/productlist')->convertLayerBlock($blockName);
        }
    }
