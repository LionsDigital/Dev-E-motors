...
class Olegnax_Colorswatches_Helper_Mediafallback extends Mage_Core_Helper_Abstract {
    ...
    protected function _resizeProductImage($product, $type, $keepFrame, $image = null, $placeholder = false)
    {
        ...
        if ($hasTypeData || $placeholder || $image) {
            ...
            if ($type == 'small_image') {
                list($sizeX, $sizeY) = $imgHelper->getMainSize();
            } else {
                $sizeX = Mage::getStoreConfig(Mage_Catalog_Helper_Image::XML_NODE_PRODUCT_BASE_IMAGE_WIDTH);
                $sizeY = $sizeX;
            }
            return (string) Mage::helper('catalog/image')->init($product, 'image')->resize($sizeX, $sizeY);
        }
        return false;
    }
    ...
    protected function _getChildrenProductsAttributes()
    {
        return array(
            ...
            'image_label',
            'small_image_label',
            Mage::helper('olegnaxcolorswatches/productlist')->getSwatchAttribute()->getAttributeCode(),
        );
    }
    ...
    public function attachChildrenProducts(array $products, $storeId)
    {
        ...
        $collection = Mage::getResourceModel(
            'olegnaxcolorswatches/catalog_product_type_configurable_product_collection');
    ...
    public function attachGallerySetToCollection(array $products, $storeId)
    {
		...
		if(!method_exists($resourceModel, 'loadGallerySet'))
		{
			$resourceModel = Mage::getResourceModel('olegnaxcolorswatches/catalog_product_attribute_media_media');			
		}		
    ...
    public function getConfigurableImagesFallbackArray(Mage_Catalog_Model_Product $product, array $imageTypes,
        ...
        if (!$product->hasConfigurableImagesFallbackArray()) {
            ...
            foreach ($mapping as $map) {
                ...
                $imagesByLabel[$map['label']] = array(
                    'configurable_product' => array(
                        Olegnax_Colorswatches_Helper_Productimg::MEDIA_IMAGE_TYPE_SMALL => null,
                        Olegnax_Colorswatches_Helper_Productimg::MEDIA_IMAGE_TYPE_BASE => null,
                    ),
                ...
                if ($imagePath) {
                    $imagesByLabel[$map['label']]['configurable_product']
                        [Olegnax_Colorswatches_Helper_Productimg::MEDIA_IMAGE_TYPE_SMALL] =
                            $this->_resizeProductImage($product, 'small_image', $keepFrame, $imagePath);

                    $imagesByLabel[$map['label']]['configurable_product']
                        [Olegnax_Colorswatches_Helper_Productimg::MEDIA_IMAGE_TYPE_BASE] =
            ...
            $array = array(
                ...
                Olegnax_Colorswatches_Helper_Productimg::MEDIA_IMAGE_TYPE_SMALL => $imagesByType['small_image'],
                Olegnax_Colorswatches_Helper_Productimg::MEDIA_IMAGE_TYPE_BASE => $imagesByType['image'],
            );
