			...
			try {
				...
				if (!$this->_getSession()->getNoCartRedirect(true)) {
					if (!$cart->getQuote()->getHasError()){
						$message = $this->__( $this->__('%s was added to your shopping cart.') .'<br/><br/> <button class="button" onclick="window.top.location.href = \''.Mage::getUrl('checkout/cart').'\';"><span><span>'. $this->__('View Cart') .'</span></span></button><button class="button" onclick="window.top.location.href = \''.Mage::helper('checkout/url')->getCheckoutUrl().'\';"><span><span>'. $this->__('Checkout') .'</span></span></button>', Mage::helper('core')->escapeHtml($product->getName()));
