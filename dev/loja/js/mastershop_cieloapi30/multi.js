 ...
 * @return void
 */
mastershop_cieloapi30_atualizarParcelasValorEspecificado = function (pElemento)
{
	var numCartao = parseInt(pElemento.id.substr(pElemento.id.lastIndexOf('_') - 1, 1));
	var elementoParcelas = $('mastershopcieloapi30_cc_' + numCartao + '_parcelas');
	...
	new Ajax.Request(
		mastershop_cieloapi30_parcelasUrl,
		{
			method: 'post',
			parameters:
			{
				valor: mastershop_cieloapi30_valorFormatadoParaNumero(pElemento.value),
				moeda: mastershop_cieloapi30_configMoeda.codigo
			},
			onError: function()
			{
				elementoParcelas.options.length = 0;
            ...
            onSuccess: function(pDados)
			{
				try
				{
					...
					for (it = 0; it < resultado.length; ++it)
					{
						...
						if (
							typeof(mastershop_cieloapi30_parcelasSel[numCartao]) != 'undefined' &&
							mastershop_cieloapi30_parcelasSel[numCartao] == it + 1
						)
						{
							opcao.selected = true;
						}
 ...
 * @return void
 */
mastershop_cieloapi30_atualizarValor = function (pEvento)
{
	...
	{
		elementoValor.toggleClassName('validar_valor_cartao');
		elementoValor.value = VMasker.toMoney(valorOriginal, mastershop_cieloapi30_configMoeda);
	...
	if (valorEspecificado != valorPorCartao && valorEspecificado != valorOriginal)
	{
		...
		mastershop_cieloapi30_atualizarParcelasValorEspecificado(pEvento.srcElement);
	}
	else
	{
		if (valorEspecificado == valorPorCartao)
		{
			...
			mastershop_cieloapi30_atualizarParcelasValoresQtd(mastershop_cieloapi30_recuperarQtdSelecionada(), indiceCartao);
		}
		else
		{
	...
	mastershop_cieloapi30_atualizarValores(indiceCartao);
};
 ...
 * @return void
 */
mastershop_cieloapi30_atualizarValores = function (pPularIndice = 0)
{
	//Dividir valor restante entre campos não modificados
	var valoresEspecificados = mastershop_cieloapi30_recuperarValoresEspecificados();
	var qtdCartoesSelecionada = mastershop_cieloapi30_recuperarQtdSelecionada();

	if (qtdCartoesSelecionada > 1)
	{
		var valorPorCartao = ((mastershop_cieloapi30_multiTotal - valoresEspecificados.totalEspecificado) / valoresEspecificados.camposNaoEspecificados.length).toFixed(2);
		...
		for (indiceCartao = 1; indiceCartao <= qtdCartoesSelecionada; ++indiceCartao)
		{
			...
			var elementoValor = $('mastershopcieloapi30_cc_' + indiceCartao + '_valor');
			...
			if (null != elementoValor)
			{
				...
				if (mastershop_cieloapi30_valorFormatadoParaNumero(elementoValor.value) == elementoValor.readAttribute('valor_por_cartao'))
				{
					mastershop_cieloapi30_atualizarParcelasValoresQtd(mastershop_cieloapi30_recuperarQtdSelecionada(), indiceCartao);
				}
				else
				{
					mastershop_cieloapi30_atualizarParcelasValorEspecificado(elementoValor);
				}
			}
		}
	}
...
mastershop_cieloapi30_recuperarValoresEspecificados = function ()
{
	var qtdCartoesSelecionada = mastershop_cieloapi30_recuperarQtdSelecionada();
	...
	for (numCartao = 1; numCartao <= qtdCartoesSelecionada; ++numCartao)
	{
		...
		if (null != elementoValor)
		{
			...
			else
			{
				...
				totalEspecificado += parseFloat(mastershop_cieloapi30_valorFormatadoParaNumero(elementoValor.value));
			}
		}
	}
 ...
 * @return float
 */
mastershop_cieloapi30_valorFormatadoParaNumero = function (pValor)
{
	return parseFloat(
		pValor.replace(mastershop_cieloapi30_configMoeda.unit, '')
		.replace(mastershop_cieloapi30_configMoeda.delimiter, '')
		.replace(mastershop_cieloapi30_configMoeda.separator, '.')
		.replace(' ', '')
...
Validation.add(
	...
	function(pValor, pElemento)
	{
		...
		for (numCartao = 1; numCartao <= qtdCartoesSelecionada; ++numCartao)
		{
			...
			if (null != elementoValor)
			{
				total += parseFloat(mastershop_cieloapi30_valorFormatadoParaNumero(elementoValor.value));
			}
		}

		var resto = mastershop_cieloapi30_multiTotal - total;

		if (mastershop_cieloapi30_configMoeda.codigo != 'BRL')
		{
			resto = (resto * mastershop_cieloapi30_configMoeda.rateBaseToBRL).toFixed(2);
		}
...
Validation.add(
	...
	function(pValor, pElemento)
	{
		...
		if (mastershop_cieloapi30_configMoeda.codigo != 'BRL')
		{
			valorMinimo = (valorMinimo / mastershop_cieloapi30_configMoeda.rateBaseToBRL).toFixed(2);
		}

		return parseFloat(mastershop_cieloapi30_valorFormatadoParaNumero(pElemento.value)) >= valorMinimo;
	}
);