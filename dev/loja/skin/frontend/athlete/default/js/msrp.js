...
Catalog.Map = {
    ...
    showHelp: function(event) {
        ...
        if (this != Catalog.Map && Catalog.Map.active != this.link) {
            ...
            if (typeof this.cartLink != 'undefined') {
				cartButton.setAttribute("data-click", this.cartLink);
                if (typeof productAddToCartForm == 'undefined' || this.notUseForm) {					
					if(!Olegnax.Ajaxcart.options.status)
					{
    ...
    bindProductForm: function(){
        if (('undefined' != typeof productAddToCartFormProductView) && productAddToCartFormProductView) {			
            productAddToCartFormOld = productAddToCartFormProductView;
            ...
            return false;
        } else if ('undefined' == typeof productAddToCartFormProductView) {
            productAddToCartForm = new VarienForm('product_addtocart_form_from_popup');
        }

        productAddToCartForm.submit = function(button, url) {			
			if($(button).hasAttribute('oxajax_map'))
			{
				if (Catalog.Map.active)
				{
					Catalog.Map.hideHelp();
				}
				if (('undefined' != typeof productAddToCartFormOld) && productAddToCartFormOld && $(button).readAttribute('data-click').indexOf("checkout/cart") == -1)
				{					
